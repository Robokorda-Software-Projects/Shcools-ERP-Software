...
const handleDeleteSchool = async (schoolId: string, schoolName: string) => {
  ...
  try {
    ...
    // IMPORTANT: Delete audit logs FIRST (they reference school_id)
    ...
    // 5. Update profiles to remove school_id (ONLY for school_admin and teacher, NOT super_admin)
    const { error: profilesError } = await supabase
      .from('profiles')
      .update({ school_id: null, updated_at: new Date().toISOString() })
      .eq('school_id', schoolId)
      .neq('role', 'super_admin')  // Never touch super_admin profiles
